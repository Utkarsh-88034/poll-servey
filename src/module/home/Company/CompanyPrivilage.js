...
const CompanyPrivilage = ({classes,sateList,modulesState,nextTabHandler,previousTabHandler}) => {
    ...
    const {navs,tabs} = companyInfoState.data.modules;
    ...
    //const [statesLengthState,setStatesLengthState] = React.useState(0);


    const [defaultSateCode,setDefaultSateCode] = React.useState(companyInfoState.defautSateCode); 
    
    const [csubsStartDate, setCSubsStartDate] = React.useState(new Date());
    const [csubsEndDate, setCSubsEndDate] = React.useState(new Date());
    ...
    const updateTabState = () => {
        const subsDate = { 
            "subsStartDate" : format(csubsStartDate,"yyyy-MM-dd")  , 
            "subsEndDate" : format(csubsEndDate,"yyyy-MM-dd") 
        };
        dispatch(companyInfoSateNavsModuleUpdate(navsState));
        dispatch(companyInfoSateTabsModuleUpdate(tabsState));
        dispatch(companyInfoStatesUpdate(Object.values(statesMapState)));               
        dispatch(companyInfoSubscriptionUpdate(subsDate));
    }  
        ...
        if(stateOthersCodeObj.length>statesLength){
            ...
            const stateMapTmp = stateCodeList.map((obj,index) =>{  
                ...
                sobj[obj] = {
                    ...
                    "stateSubsInfo":{
                        "subsStartDate": format(subsStartDate,"yyyy-MM-dd"),
                        "subsEndDate": format(subsEndDate,"yyyy-MM-dd")
                     },
                     "isActive":true,
                   }
        ...
        <div>
            <Paper style={{ padding: 16 }} variant="outlined" square  >
                ...
                <Grid item xs={12} >
                    ...
                    <Grid item xs={6}>
                        <InputLabel className={classes.marginTop2} id="company-other-mutiple-checkbox-label">Select Other's State</InputLabel>
                        <Select
                            labelId="company-other-mutiple-checkbox-label"
                            id="company-other-mutiple-checkbox"
                            multiple
...
export default CompanyPrivilage;
